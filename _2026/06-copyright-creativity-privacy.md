---
lecture: 6
title: "AI, Copyright, Creativity, and Privacy"
nav_title: "Copyright, Creativity, Privacy"
date: 2026-10-12
unit: 3
unit_title: "Individual Risks"
ready: true
# This session merges what the F26 syllabus ran as two separate lectures —
# "AI, Copyright, and Creativity" and "Privacy in AI". A combined reading list
# has not been set yet, so the box renders an explicit TBD. The two superseded
# lists are preserved verbatim below so no citation has to be recovered from
# git history when the merged list is chosen; delete them once it is.
readings_tbd: true
#
# --- Superseded: "AI, Copyright, and Creativity" (was Lecture 8, Mon Oct 19) ---
# summary: This lecture examines how AI model training intersects with copyright
#   law, focusing on disputes like OpenAI v. The New York Times. We'll explore
#   distinctions between output similarity and the use of copyrighted training
#   data, and assess potential legal consequences for model developers. Technical
#   issues such as dataset curation, memorization, unlearning, and output
#   replication will be discussed alongside emerging attribution mechanisms like
#   output tracing. In addition, AI raises new questions about human creativity
#   and potential.
# readings:
#   required:
#     - authors: "Congressional Research Service"
#       year: 2025
#       title: "Generative Artificial Intelligence and Copyright Law"
#       venue: "June 2025"
#       pages: 7
#     - authors: "Liu, et al."
#       year: 2025
#       title: "Language Models May Verbatim Complete Text They Were Not Explicitly Trained On"
#       venue: "International Conference on Machine Learning (ICML)"
#       pages: 9
#     - authors: "Ippolito, D., et al."
#       year: 2023
#       title: "Preventing Verbatim Memorization in Language Models"
#       venue: "Association for Computational Linguistics (ACL)"
#       pages: 9
#     - authors: "Cooper, A., et al."
#       year: 2024
#       title: "Machine Unlearning Doesn't Do What You Think"
#       pages: 24
#     - authors: "Lemley, M."
#       year: 2024
#       title: "How Generative AI Turns Copyright Law Upside Down"
#       venue: "Science and Technology Law Review, 25(2)"
#       pages: 23
#     - authors: "Cooper, A., & Grimmelmann, J."
#       year: 2025
#       title: "On Copyright, Memorization, and Generative AI"
#       venue: "Chicago-Kent Law Review, 100(1), 141–217"
#       pages: 76
#
# --- Superseded: "Privacy in AI" (was Lecture 7, Wed Oct 14) ---
# summary: This session will cover privacy, which can be implicated in the use of
#   private data in AI model training, the use of AI to discover or disclose
#   private information about a person, and the use of AI in surveillance.
# readings:
#   required:
#     - authors: "Burgess, M."
#       year: 2023
#       title: "ChatGPT Has a Big Privacy Problem"
#       venue: "Wired"
#       pages: 8
#     - authors: "NOYB"
#       year: 2024
#       title: "ChatGPT Provides False Information About People, and OpenAI Can't Correct It"
#       venue: "April 29, 2024"
#       pages: 3
#     - authors: "NOYB"
#       year: 2024
#       title: "NOYB Urges 11 DPAs to Immediately Stop Meta's Abuse of Personal Data for AI"
#       venue: "June 6, 2024"
#       pages: 5
#     - authors: "European Data Protection Board"
#       year: 2024
#       title: "Opinion 28/2024 on Certain Data Protection Aspects Related to the Processing of Personal Data in the Context of AI Models"
#       pages: 35
#     - authors: "Kibby, C., & Sentinella, D."
#       year: 2024
#       title: "New Laws in California Look to the Future of Privacy and AI"
#       venue: "International Association of Privacy Professionals (IAPP)"
#       pages: 5
#     - authors: "Solove, D."
#       year: 2025
#       title: "Artificial Intelligence and Privacy"
#       pages: 73
---
