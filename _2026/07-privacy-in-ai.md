---
lecture: 7
title: "Privacy in AI"
nav_title: "Privacy in AI"
date: 2026-10-14
week: 4
week_title: "Governance Challenges, Part 2: Privacy, Ownership, and Creative Work"
deadline: "Milestone 1 due"
deadline_date: "Fri Oct 16"
ready: true
summary: >-
  This session will cover privacy, which can be implicated in the use of private
  data in AI model training, the use of AI to discover or disclose private
  information about a person, and the use of AI in surveillance.
readings:
  required:
    - authors: "Burgess, M."
      year: 2023
      title: "ChatGPT Has a Big Privacy Problem"
      venue: "Wired"
      pages: 8
    - authors: "NOYB"
      year: 2024
      title: "ChatGPT Provides False Information About People, and OpenAI Can't Correct It"
      venue: "April 29, 2024"
      pages: 3
    - authors: "NOYB"
      year: 2024
      title: "NOYB Urges 11 DPAs to Immediately Stop Meta's Abuse of Personal Data for AI"
      venue: "June 6, 2024"
      pages: 5
    - authors: "European Data Protection Board"
      year: 2024
      title: "Opinion 28/2024 on Certain Data Protection Aspects Related to the Processing of Personal Data in the Context of AI Models"
      pages: 35
    - authors: "Kibby, C., & Sentinella, D."
      year: 2024
      title: "New Laws in California Look to the Future of Privacy and AI"
      venue: "International Association of Privacy Professionals (IAPP)"
      pages: 5
    - authors: "Solove, D."
      year: 2025
      title: "Artificial Intelligence and Privacy"
      pages: 73
---
